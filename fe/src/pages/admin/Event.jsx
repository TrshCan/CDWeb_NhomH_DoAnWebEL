...
const EventModal = ({ modalData, closeModal, handleSave }) => {
  ...
  const validate = () => {
    ...
    if (!/^[a-zA-Z0-9\sÀ-ỹ.,-]*$/.test(formData.title))
      newErrors.title = "Tiêu đề chứa ký tự không hợp lệ.";
    if (formData.location && !/^[a-zA-Z0-9\sÀ-ỹ.,-]*$/.test(formData.location))
...
export default function EventManager() {
  ...
  const handleSaveEvent = async (formData) => {
    try {
      ...
      if (error.message.includes("validation") || error.message.includes("auth") || error.message.includes("permission")) {
        // Check if it's a validation error with JSON
        try {
          const backendErrors = JSON.parse(error.message);
          if (backendErrors.title || backendErrors.event_date || backendErrors.location) {
            setErrors({
        ...
        if (error.message.includes("Bạn chưa đăng nhập") || error.message.includes("auth")) {
          ...
          errorMessage = error.message || "Bạn không có quyền thực hiện thao tác này.";
        } else {
